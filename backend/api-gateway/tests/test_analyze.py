...
def test_analyze_success_with_mocked_document_parser():
    ...
    with patch(
        ...
        response = client.post(
            "/api/v1/analyze",
            files=files,
            data={"jd_text": "Looking for Python and FastAPI skills."},
        )
    ...
    assert data["jd_text_length"] == len("Looking for Python and FastAPI skills.")
...
def test_analyze_when_document_parser_fails():
    ...
    with patch(
        ...
        response = client.post(
            "/api/v1/analyze",
            files=files,
            data={"jd_text": "Looking for Python skills."},
        )
